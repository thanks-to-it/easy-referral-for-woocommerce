...
if ( ! class_exists( 'ThanksToIT\ERWC\Shortcodes' ) ) {
	...
	class Shortcodes {
		...
		function init() {
			...
			add_filter( 'erwc_referral_codes_table_column_value', array( $this, 'get_erwc_referral_codes_table_column_value' ), 10, 4 );
			...
			// Sharer
			add_shortcode( 'erwc_sharer', array( $this, 'erwc_sharer' ) );
		}

		function erwc_sharer( $atts = null ) {
		...
		/**
		 * get_erwc_referral_codes_table_column_value.
		 *
		 * @version 1.0.6
		 * @since   1.0.6
		 *
		 ...
		 * @throws \ReflectionException
		 */
		function get_erwc_referral_codes_table_column_value( $value, $column, $code_id, $code ) {
			...
			switch ( $column ) {
				case 'code':
					$value = $referrer_code;
					break;
				case 'referral_code_url':
					$value = '<a href="'.esc_url( $code_manager->get_referrer_code_url( $referrer_code ) ).'">'.esc_url( $code_manager->get_referrer_code_url( $referrer_code ) ).'</a>';
					break;
				case 'copy':
					$value = '<button data-url="'.esc_url( $code_manager->get_referrer_code_url( $referrer_code ) ).'" class="erwc-action-btn erwc-copy-link-btn"><i class="dashicons dashicons-clipboard clipboard"></i><i class="dashicons dashicons-yes yes"></i></button>';
					break;
			}
			return $value;
		}
		 ...
		 *
		 * @todo Remove some columns for email
		 *
		 * @return array
		 */
				...
				<table <?php echo $table_params ?> class="td erwc-referrals-table my_account_orders shop_table">
					...
					<tr>
						...
						<?php foreach ( $columns as $column ): ?>
							<th class="td" scope="col"><?php echo esc_html( $column ) ?></th>
						<?php endforeach; ?>
					</tr>
					</thead>
					<tbody>
		...
		function erwc_referral_codes_table( $atts ) {
			...
			<table class="erwc-referral-codes-table my_account_orders shop_table">
				<thead>
				<tr>
					<?php $columns = apply_filters( 'erwc_referral_codes_table_columns', array(
						'code'              => __( 'Code', 'easy-referral-for-woocommerce' ),
						'referral_code_url' => __( 'Referral Code URL', 'easy-referral-for-woocommerce' ),
						'copy'              => __( 'Copy', 'easy-referral-for-woocommerce' ),
					), $atts ); ?>
					<?php foreach ( $columns as $column ): ?>
						<th class="td" scope="col"><?php echo esc_html( $column ) ?></th>
					<?php endforeach; ?>
				...
				<?php foreach ( $codes as $code_id => $code ): ?>
					...
					<tr>
						<?php foreach ( $columns as $column_key => $column ): ?>
							<td class="td"><?php echo apply_filters( 'erwc_referral_codes_table_column_value', '', $column_key, $code_id, $code ); ?></td>
						<?php endforeach; ?>
					</tr>
				<?php endforeach; ?>
				</tbody>
